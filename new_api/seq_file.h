...
#define TERMINATE_BUFFER(buf) (buf.b[buf.end] = 0)

#define CHOMP_BUFFER(buf) do { \
...
#define _func_read_sam() \
  static int8_t seq_comp_table[16] = { 0, 8, 4, 12, 2, 10, 9, 14,              \
                                       1, 6, 5, 13, 3, 11, 7, 15 };            \
                                                                               \
  int sread_s(seq_file_t *sf, read_t *read)                                    \
  {                                                                            \
    read->name.end = read->seq.end = read->qual.end = 0;                       \
    TERMINATE_BUFFER(read->name);                                              \
    TERMINATE_BUFFER(read->seq);                                               \
    TERMINATE_BUFFER(read->qual);                                              \
                                                                               \
...
#define _func_read(fname,__getc,__readline) \
  int fname(seq_file_t *sf, read_t *read)                                      \
  {                                                                            \
    read->name.end = read->seq.end = read->qual.end = 0;                       \
    TERMINATE_BUFFER(read->name);                                              \
    TERMINATE_BUFFER(read->seq);                                               \
    ...
    {                                                                          \
      ...
      if(c == -1) return -1;                                                   \
      do {                                                                     \
        if(__readline(sf,read->qual) > 0) CHOMP_BUFFER(read->qual);            \
        else return 1;                                                         \
...
#define SETUP_SEQ_FILE() \
  ...
  _func_close()                                                                \
  _func_reads()                                                                \
  _func_get_qual_limits()                                                      \
  _func_read_valid(seq_read_looks_valid_dna,"acgtn")                           \
  _func_read_valid(seq_read_looks_valid_rna,"acgun")                           \
  _func_read_valid(seq_read_looks_valid_protein,"acdefghiklmnopqrstuvwy")      \
